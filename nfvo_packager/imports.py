...
class ImportsLoader(object):
    ...
    def __init__(self, importslist, path, type_definition_list=None,
        ...
        if not path and not tpl:
            msg = 'Input tosca template is not provided'
            log.warning(msg)
    ...
    def _validate_and_load_imports(self):
        ...
        if not self.importslist:
            msg = '"imports" keyname is defined without including templates'
            log.error(msg)
            raise RuntimeError(msg)
            ...
            if isinstance(import_def, dict):
                for import_name, import_uri in import_def.items():
                    if import_name in imports_names:
                        msg = 'Duplicate import name "%s" was found' % (
                            import_name)
                        log.error(msg)
                        raise RuntimeError(msg)
    ...
    def _validate_import_keys(self, import_name, import_uri_def):
        if self.FILE not in import_uri_def.keys():
            raise RuntimeError(
                'Missing keyname "file" in import "%(name)s"'
                % {'name': import_name})
        for key in import_uri_def.keys():
            if key not in self.IMPORTS_SECTION:
                msg = 'Unknown keyname "%(key)s" error in ' \
    ...
    def _load_import_template(self, import_name, import_uri_def):
        ...
        if isinstance(import_uri_def, dict):
            ...
            if repository is not None:
                if repository not in repos:
                    raise RuntimeError(
                        'Repository is not found in "%s"' % repos)
        else:
        ...
        if not file_name:
            msg = 'A template file name is not provided with import ' \
                  'definition "%(import_name)s".' \
                  % {'import_name': import_name}
            log.error(msg)
            raise RuntimeError(msg)

        if UrlUtils.validate_url(file_name):
            return file_name, load_yaml(file_name, False)
        elif not repository:
            import_template = None
            if self.path:
                if UrlUtils.validate_url(self.path):
                    if os.path.isabs(file_name):
                        msg = 'Absolute file name "%(name)s" cannot be ' \
                              'used in a URL-based input template ' \
                              '"%(template)s"' \
                              % {'name': file_name, 'template': self.path}
                        log.error(msg)
                        raise RuntimeError(msg)
                    import_template = UrlUtils.join_url(self.path, file_name)
                    a_file = False
                else:
                    a_file = True
                ...
                if os.path.isabs(file_name) and os.path.isfile(file_name):
                    ...
                    msg = 'Relative file name "%(name)s" cannot be used ' \
            ...
            if not import_template:
                raise RuntimeError(
                    'Import "%(name)s" is not valid' %
                    {'name': import_uri_def})
            return import_template, load_yaml(import_template, a_file)

        if short_import_notation:
            raise RuntimeError('Import "%(name)s" is not valid'
                               % import_uri_def)
        ...
        if repository:
            ...
            if not full_url:
                raise RuntimeError(
                    'referenced repository "%(n_uri)s" in import '
                    'definition "%(tpl)s" not found'
                    % {'n_uri': repository, 'tpl': import_name})

        if UrlUtils.validate_url(full_url):
            return full_url, load_yaml(full_url, False)
        else:
            raise RuntimeError(
